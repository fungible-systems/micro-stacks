...
type TypedArray = Uint32Array | Uint8Array | (number | bigint)[];
...
const slice = (arr: TypedArray, start = 0, end = arr.length): Uint32Array => {
...
const readLE32 = (ptr: TypedArray, padding = 0): number =>
...
const writeLE64 = (ptr: bigint[], padding: number, x: bigint | number): void => {
  x = BigInt(x);
  ptr[padding + 7] = x >> 56n;
  ptr[padding + 6] = x >> 48n;
  ptr[padding + 5] = x >> 40n;
...
const Round = (
  ...
  k: number,
  r: number
): Uint32Array => new Uint32Array([rol(a + f + x + k, r) + e, rol(c, 10)]);
