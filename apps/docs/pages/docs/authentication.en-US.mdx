...
```typescript filename="wallet-connect-button.tsx"
...
```svelte filename="wallet-connect-button.svelte"
<script lang="ts">
...
```vue filename="wallet-connect-button.vue"
...
```tsx filename="wallet-connect-button.tsx"
...
```svelte filename="account.svelte"
<script lang="ts">
  import { getAccount } from "@micro-stacks/svelte";
...
api route, check out these handy resources for working with Stacks apps & server rendering:
