...
<Integrations.Vanilla>
  <Callout emoji="⚛">
    If you are building an app using a framework like React, Svelte, Vue, or Solid.js, it is{' '}
    <strong>strongly recommended</strong> that you make use of the official integrations.
  </Callout>
</Integrations.Vanilla>
...
The primary way you'll implement authentication is via the `getAuth` function. This function exposes a few callbacks for
different functions: `openAuthRequest`, `signOut`, along with some helper reactive variables: `isRequestPending` `isSignedIn`.
...
This function will return a _reactive_ (will automatically update as state changes) object with the shape:
...
This hook will return a _reactive_ (will automatically update as state changes) object with the shape:
