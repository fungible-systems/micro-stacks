...
```svelte {2,6-7} filename="app.svelte"
<script lang="ts">
...
```svelte {3,5-8} filename="app.svelte"
<script lang="ts">
  // App.svelte file
...
```tsx filename="micro-stacks-client.ts"
