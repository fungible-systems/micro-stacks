...
export class Storage {
  ...
  constructor(options: {
    ...
    if (options.gaiaConfig && options.client)
      throw Error(
        'Both client and configurations for gaia have been passed, pass gaiaConfig options via the client'
      );
  ...
  private getGaiaHubConfig = async (privateKey: string): Promise<GaiaHubConfig> => {
    ...
    const opts = {
      ...
      gaiaReadUrl: this.gaiaConfig?.gaiaReadUrl ?? DEFAULT_GAIA_READER_URL,
      privateKey,
    };
