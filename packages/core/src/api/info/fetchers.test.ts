...
const CORE_NODE_INFO_RESPONSE_OBJ = JSON.parse(
  JSON.stringify(CORE_NODE_INFO_RESPONSE)
);
const SERVER_STATUS_RESPONSE_OBJ = JSON.parse(
  JSON.stringify(SERVER_STATUS_RESPONSE)
);
const NETWORK_BLOCK_TIMES_RESPONSE_OBJ = JSON.parse(
  JSON.stringify(NETWORK_BLOCK_TIMES_RESPONSE)
);
const NETWORK_BLOCK_TIME_RESPONSE_OBJ = JSON.parse(
  JSON.stringify(NETWORK_BLOCK_TIME_RESPONSE)
);
const GET_STX_SUPPLY_RESPONSE_OBJ = JSON.parse(
  JSON.stringify(GET_STX_SUPPLY_RESPONSE)
);
const GET_STX_SUPPLY_LEGACY_FORMAT_RESPONSE_OBJ = JSON.parse(
  JSON.stringify(GET_STX_SUPPLY_LEGACY_FORMAT_RESPONSE)
);
const CORE_NODE_POX_RESPONSE_OBJ = JSON.parse(
  JSON.stringify(CORE_NODE_POX_RESPONSE)
);
...
export const GET_INFO_MOCKS = [
  ...
  rest.get(stxSupplyLegacyFormatEndpoint(HIRO_TESTNET_DEFAULT), (_req, res, ctx) => {
    return res(ctx.json(GET_STX_SUPPLY_LEGACY_FORMAT_RESPONSE_OBJ));
  }),
  rest.get(poxEndpoint(HIRO_TESTNET_DEFAULT), (_req, res, ctx) => {
    return res(ctx.json(CORE_NODE_POX_RESPONSE_OBJ));
  }),
];
...
describe('info fetchers', () => {
  ...
  test(fetchCoreApiInfo.name, async () => {
    ...
    expect(data).toEqual(CORE_NODE_INFO_RESPONSE_OBJ);
  });

  test(fetchStatus.name, async () => {
    const args = { url: HIRO_TESTNET_DEFAULT };
    const data = await fetchStatus(args);
    expect(data).toEqual(SERVER_STATUS_RESPONSE_OBJ);
  });

  test(fetchNetworkBlockTimes.name, async () => {
    const args = { url: HIRO_TESTNET_DEFAULT };
    const data = await fetchNetworkBlockTimes(args);
    expect(data).toEqual(NETWORK_BLOCK_TIMES_RESPONSE_OBJ);
  });
  ...
  test(fetchNetworkBlockTime.name, async () => {
    ...
    expect(data).toEqual(NETWORK_BLOCK_TIME_RESPONSE_OBJ);
  });

  test(fetchStxSupply.name, async () => {
    const args = { url: HIRO_TESTNET_DEFAULT };
    const data = await fetchStxSupply(args);
    expect(data).toEqual(GET_STX_SUPPLY_RESPONSE_OBJ);
  });
  ...
  test(fetchStxSupplyLegacyFormat.name, async () => {
    ...
    expect(data).toEqual(GET_STX_SUPPLY_LEGACY_FORMAT_RESPONSE_OBJ);
  });

  test(fetchPox.name, async () => {
    const args = { url: HIRO_TESTNET_DEFAULT };
    const data = await fetchPox(args);
    expect(data).toEqual(CORE_NODE_POX_RESPONSE_OBJ);
  });
});
