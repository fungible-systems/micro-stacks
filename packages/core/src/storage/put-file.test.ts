...
import { mockFetch, mockGet, mockPut } from 'vi-fetch';
...
describe(putFile.name, () => {
  ...
  test('putFile unencrypted, using Blob content', async () => {
    ...
    try {
      ...
      const uploadToGaiaHub = vi.fn().mockResolvedValue({
        publicURL: fullReadUrl,
      });
      vi.spyOn(hub, 'uploadToGaiaHub').mockImplementation(uploadToGaiaHub);
  ...
  test('putFile encrypted, using Blob content, encrypted', async () => {
    ...
    try {
      ...
      vi.spyOn(hub, 'getFullReadUrl').mockImplementation(vi.fn().mockResolvedValue(fullReadUrl));
      ...
      mockGet(/.*/).willResolve(encryptedContent);
  ...
  // test('putFile unencrypted, using TypedArray content, encrypted', async () => {
    // try {
      ...
      // const encryptOptions: PutFileOptions = {
        ...
        // privateKey,
      // };
      // const decryptOptions: GetFileOptions = { decrypt: false, gaiaHubConfig, privateKey,
      // };
      // put and encrypt the file
      // const publicURL = await putFile(path, fileContent, encryptOptions);
  ...
  test('putFile encrypt/no-sign using specifying public key & getFile decrypt', async () => {
    ...
    mockPut(/.*/).willResolve(await encryptContent(fileContent, { publicKey }));
    mockGet(/.*/).willResolve(await encryptContent(fileContent, { publicKey }));
