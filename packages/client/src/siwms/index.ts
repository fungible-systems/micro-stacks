...
export class SignInWithStacksMessage {
  ...
  constructor(param: string | StacksMessageParams) {
    if (typeof param === 'string') param = new ParsedMessage(param);
   ...
   */
  async verify(params: VerifyParams): Promise<SiwsResponse> {
    return new Promise<SiwsResponse>(async (resolve, reject) => {
      ...
      try {
        ...
        if (addr !== this.address) {
          assert({
            ...
            data: this,
            error: new SiwsError(
              SiwsErrorType.INVALID_SIGNATURE,
              addr,
  ...
  private validateMessage(...args: any) {
    ...
    if (!this.domain || this.domain.length === 0 || !/[^#?]*/.test(this.domain)) {
      throw new SiwsError(SiwsErrorType.INVALID_DOMAIN, `${this.domain} to be a valid domain.`);
    }

    /** `address` check. */
    if (!validateStacksAddress(this.address)) {
      throw new SiwsError(SiwsErrorType.INVALID_ADDRESS, `${this.address} to be a valid address.`);
    }

    /** Check if the URI is valid. */
    if (!uri.isUri(this.uri)) {
      throw new SiwsError(SiwsErrorType.INVALID_URI, `${this.uri} to be a valid uri.`);
    }

    /** Check if the version is 1. */
    if (this.version !== '1') {
      throw new SiwsError(SiwsErrorType.INVALID_MESSAGE_VERSION, '1', this.version);
    }
